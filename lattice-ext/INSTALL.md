# Installing the Lattice Firefox Extension

## Install the extension

Preferred:

1. Open the AMO listing: https://addons.mozilla.org/en-US/firefox/addon/lattice/
2. Install the add-on normally

Alternative:

1. Open the GitHub releases page: https://github.com/fordz0/lattice/releases
2. Find the latest `lattice-ext-v*` release
3. Download the signed `lattice-extension-<version>.xpi` asset when it is attached for that AMO-approved version

The GitHub `.xpi` path is meant for direct self-distribution. AMO is still the
default recommended install path. GitHub mirrors the signed `.xpi` only when
AMO exposes a downloadable file for that release version.

Chromium preview build:

1. Run the extension release workflow with `browser=chrome` or `browser=both`
2. Download the `lattice-extension-chrome-<version>.zip` release asset

This is only a packaging target today. It is not a full Chrome Web Store-ready
Manifest V3 port yet.

Development / temporary install:

1. Open Firefox and go to about:debugging
2. Click "This Firefox"
3. Click "Load Temporary Add-on"
4. Select lattice-ext/manifest.json

The setup page will open automatically.

## One-time Firefox config

Firefox needs to know .loom is a URL, not a search query.
The setup page walks you through it - takes about 30 seconds.

Or do it manually:
1. Paste `about:config` into the Firefox address bar
2. Search: browser.fixup.domainsuffixwhitelist.loom
3. Click + to create it (defaults to true)

## Trust the local HTTPS CA

Lattice serves `.loom` sites over local HTTPS via the local daemon/proxy
while keeping `https://name.loom` in the address bar.
To remove security warnings, import the local CA certificate generated by
`lattice-daemon`:

1. Run `lattice up`
2. Run `lattice doctor` if you want to confirm the daemon and local ports are healthy before touching Firefox certificates
3. Download `http://127.0.0.1:7781/__lattice_ca.pem`
4. Paste `about:preferences#privacy` into the Firefox address bar
5. Firefox: Settings → Privacy & Security → Certificates → View Certificates
6. Authorities tab → Import → select `lattice-local-ca.pem`
7. Enable trust for websites

## Requirements

- lattice-daemon must be running on the default ports (7779/7780/7781/7443/7782)
- Firefox 140 or later

## Usage

Type any .loom address in the Firefox address bar:
  lattice.loom
  https://lattice.loom
